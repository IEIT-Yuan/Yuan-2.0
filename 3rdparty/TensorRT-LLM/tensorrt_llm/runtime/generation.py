...
class GenerationSession(object):
    ...
    def setup(self,
        ...
        lf1_cache_shape = (
            batch_size,
            self.num_heads_kv * self.head_size,
            1, 1
        )
        lf2_cache_shape = (
            batch_size,
            self.num_heads_kv * self.head_size // 2,
            1, 1
        )
        for i in range(self.first_layer, self.last_layer):
            ...
            self.buffer[f'present_lf1_{i}'] = torch.zeros(lf1_cache_shape, dtype=kv_cache_type, device=self.device)
            self.buffer[f'present_lf2_{i}'] = torch.zeros(lf2_cache_shape, dtype=kv_cache_type, device=self.device)
            if self.cross_attention:
    ...
    def _get_context_shape_buffer(
        ...
        for idx in range(self.first_layer, self.last_layer):
            lf1_cache = self.buffer[f'present_lf1_{idx}']
            add_tensor(lf1_cache, f'past_lf1_{idx}')
            add_tensor(lf1_cache, f'present_lf1_{idx}')

            lf2_cache = self.buffer[f'present_lf2_{idx}']
            add_tensor(lf2_cache, f'past_lf2_{idx}')
            add_tensor(lf2_cache, f'present_lf2_{idx}')
