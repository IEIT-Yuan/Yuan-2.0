...
class GenerationMixin:
    ...
    def prepare_basic_inputs(self,
        ...
        for i in range(num_layers):
            lf1_dim_range = OrderedDict([
                ('batch_size_beam_width', bb_range),
                ('lf1_size', [[head_size * num_heads, head_size * num_heads, head_size * num_heads]]*2),
                ('lf1_dim1', [[1, 1, 1]]*2),
                ('lf1_dim2', [[1, 1, 1]]*2),
            ])
            lf1 = Tensor(name=f'past_lf1_{i}',
                        dtype=kv_dtype,
                        shape=[-1, num_kv_heads * head_size, 1, 1],
                        dim_range=lf1_dim_range)
            past_lf1.append(lf1)
            lf2_dim_range = OrderedDict([
                ('batch_size_beam_width', bb_range),
                ('lf2_size', [[head_size * num_heads // 2, head_size * num_heads // 2, head_size * num_heads // 2]]*2),
                ('lf2_dim1', [[1, 1, 1]]*2),
                ('lf2_dim2', [[1, 1, 1]]*2),
            ])
            lf2 = Tensor(name=f'past_lf2_{i}',
                        dtype=kv_dtype,
                        shape=[-1, num_kv_heads * head_size // 2, 1, 1],
                        dim_range=lf2_dim_range)
            past_lf1.append(lf1)
            past_lf2.append(lf2)
