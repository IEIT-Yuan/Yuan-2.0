...
class MegaMultiDimensionDampedLFA(nn.Module):
    """
    ...
    def __init__(self, hidden_size):
        ...
        self.lfa_conv2d_group = 1
        self.lfa_conv2d_num_pad = 1
        self.conv1 = torch.nn.Conv2d(self.embed_dim, self.embed_dim // 2, (2, 1), stride=(1, 1), padding=(self.lfa_conv2d_num_pad, 0), groups=self.lfa_conv2d_group)
        self.conv2 = torch.nn.Conv2d(self.embed_dim // 2, self.embed_dim, (2, 1), stride=(1, 1), padding=(self.lfa_conv2d_num_pad, 0), groups=self.lfa_conv2d_group)
        self.output_layernorm = LlamaRMSNorm(self.embed_dim)


    def forward(self, inputs, pre_hidden_states):
        assert self.lfa_conv2d_num_pad == 1
        if pre_hidden_states is None:
            ...
            ema_output = self.output_layernorm(output2 + residual)
            ema_output = ema_output.transpose(0,1)
            return ema_output
        else:
            inputs = inputs.transpose(0,1)
            pre_hidden_states = pre_hidden_states.transpose(0,1)
            residual = inputs
            ...
            lfa_output = self.output_layernorm(output2 + residual)
            lfa_output = lfa_output.transpose(0,1)
...
class YuanAttention(nn.Module):
    ...
    def __init__(self, config: YuanConfig):
        ...
        self.ema_gate = MegaMultiDimensionDampedLFA(self.hidden_size)
    ...
    def forward(
        ...
        hidden_states = self.lfa_gate(hidden_states, pre_hidden_states)
