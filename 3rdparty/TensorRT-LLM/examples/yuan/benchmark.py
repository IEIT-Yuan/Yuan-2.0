    ...
    for i, (prompt, prompt_len, new_token_len) in tqdm(enumerate(requests),
        ...
        for i in range(len(batch)):
            ...
            for i in range(len(temp_ids)):
                if temp_ids[i] in [tokenizer.bos_token_id, tokenizer.eos_token_id]:
...
def run_hf(
    ...
    if llama.config.model_type == "llama":
        # To enable padding in the HF backend.
        tokenizer.pad_token = tokenizer.eos_token
    llama = llama.cuda()
    # Assuming llama model requires specific token IDs for end-of-sequence or padding
    ...
    for i in range(len(requests)):
        ...
        output_texts = tokenizer.batch_decode(pure_output_ids, skip_special_tokens=True)
        output_lengths = [len(out_ids) for out_ids in pure_output_ids]
