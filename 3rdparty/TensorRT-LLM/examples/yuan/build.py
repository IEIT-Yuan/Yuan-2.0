...
#from modeling_yuan import YuanForCausalLM
...
from weight import (get_scaling_factors, load_from_awq_yuan, load_from_gptq_yuan, load_from_hf_yuan)
...
def parse_arguments():
    ...
    parser.add_argument('--model_dir', type=str, default="/temp_data/LLM_test/yuan_model")
    ...
    parser.add_argument('--vocab_size', type=int, default=135040)
    ...
    parser.add_argument('--ffn_dim_multiplier', type=float, default=4.0)
    ...
    parser.add_argument(
        ...
        '(0.45734 vs 0.45755 for YUAN-v2 7B using ammo/examples/hf/instruct_eval/mmlu.py).'
    )
    ...
    if args.model_dir is not None:
        hf_config = YuanConfig.from_pretrained(args.model_dir)
        args.inter_size = hf_config.intermediate_size  # override the inter_size for Yuan
        ...
        if hf_config.model_type == "mixtral":
            # HF YUAN-type models are implicitly using gated activation.
        ...
        args.inter_size = inter_size  # override the inter_size for Yuan
...
def build_rank_engine(builder: Builder,
    ...
    # Initialize Module
    tensorrt_llm_yuan = tensorrt_llm.models.YuanForCausalLM(
    ...
    if args.use_weight_only or args.use_smooth_quant or args.enable_fp8 or args.fp8_kv_cache:
        tensorrt_llm_yuan = quantize_model(tensorrt_llm_yuan, args.quant_mode, **quantize_kwargs)
    if args.per_group:
        load_func = load_from_awq_yuan if args.weight_only_precision == 'int4_awq' else load_from_gptq_yuan
        load_func(tensorrt_llm_yuan=tensorrt_llm_yuan,
                  quant_ckpt_path=args.quant_ckpt_path,
                  mapping=mapping,
                  dtype=args.dtype,
                  bin_model_dir=args.bin_model_dir)
    else:
        logger.info(f'Loading HF Yuan ... from {args.model_dir}')
        tik = time.time()
        hf_yuan = YuanForCausalLM.from_pretrained(
            args.model_dir,
            device_map={
                "model": "cpu",
                "lm_head": "cpu",
        ...
        load_from_hf_yuan(tensorrt_llm_yuan,
                           hf_yuan,
                           mapping=mapping,
                           dtype=args.dtype,
        ...
        tok = time.time()
        t = time.strftime('%H:%M:%S', time.gmtime(tok - tik))
        logger.info(f'HF Yuan loaded. Total time: {t}')
    ...
    with net_guard(network):
        # Prepare
        network.set_named_parameters(tensorrt_llm_yuan.named_parameters())

        # Forward
        inputs = tensorrt_llm_yuan.prepare_inputs(args.max_batch_size,
                                                   args.max_input_len,
        ...
        tensorrt_llm_yuan(*inputs)
        if args.enable_debug_output:
            # mark intermediate nodes' outputs
            for k, v in tensorrt_llm_yuan.named_network_outputs():
                v = v.trt_tensor
                v.name = k
